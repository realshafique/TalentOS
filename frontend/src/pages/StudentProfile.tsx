...
function StudentProfile() {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        ...
        const profiles: Student[] = response.data;

        const foundStudent = profiles.find(
          (profile) => profile.id === Number(studentId)
        );
  ...
  const fetchTeams = async () => {
    try {
      ...
      alert("Unable to load your teams.");
    } finally {
  ...
  const addToTeam = async () => {
    ...
    try {
      ...
      alert(`${student.name} was added to the team successfully!`);
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.data?.detail) {
          ...
          alert("Unable to add student to the team.");
        }
      } else {
      ...
      <main className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
          ...
          <div className="mt-8 border-t border-slate-100 pt-6">
            {!showTeamForm ? (
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Select one of your teams and define this student's role.
                </p>
        ...
        <section className="mt-6 rounded-xl border border-slate-200 bg-white p-6 sm:p-8">
          ...
          <p className="mt-4 whitespace-pre-line text-sm leading-7 text-slate-600">
            {student.about || "No description provided."}
          </p>
        </section>
